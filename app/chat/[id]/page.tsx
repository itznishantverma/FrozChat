...
export default function ChatPage() {
  ...
  useEffect(() => {
    if (partnerUser) {
      const displayName = partnerUser.display_name || partnerUser.username || 'Anonymous'
      const unreadText = totalUnreadCount > 0 ? `(${totalUnreadCount}) ` : ''
      const newTitle = `${unreadText}FrozChat • ${displayName}`
      console.log('[Page] Updating document title to:', newTitle)
  ...
  const updateUnreadCount = (count: number) => {
    console.log('[Page] Received unread count update:', count)
