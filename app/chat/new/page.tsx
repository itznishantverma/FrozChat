...
export default function ChatNewPage() {
  ...
  const handleStartMatching = async () => {
    ...
    if (activeChatRoomId) {
      ...
      toast({
        title: 'Active Chat Found',
        description: 'Redirecting you to your active chat...',
      })
      router.push(`/chat/${activeChatRoomId}`)
      return
    }
    ...
    try {
      ...
      matchSubscription = MatchingService.subscribeToMatches(
        ...
        async (match) => {
          ...
          if (match && match.id && match.chat_room_id) {
            ...
            console.log('🚀 Redirecting to chat room:', match.chat_room_id)
            setSearching(false)
      ...
      if (immediateMatch) {
        ...
        if (!roomId) {
          console.log('Creating new chat room for match...')
        ...
        if (roomId) {
          console.log('🚀 Redirecting to chat room (immediate match):', roomId)
          setSearching(false)
          router.push(`/chat/${roomId}`)
          return
        } else {
          console.error('❌ Failed to get room ID for immediate match')
        }
      } else {
        console.log('No immediate match found, waiting for subscription...')
      }

      // Fallback: Periodically check for match in case realtime fails
      matchCheckInterval = setInterval(async () => {
        ...
        if (activeMatch && activeMatch.chat_room_id) {
          ...
          setSearching(false)
          router.push(`/chat/${activeMatch.chat_room_id}`)
        }
      }, 2000)
